...
static force_inline void draw_scores(surface_t* disp, u32 score1, u32 score2)
{
    ...
    graphics_draw_text(disp, SCREEN_WIDTH/2 - 20, 10, p1);

    static char p2[8];
    sprintf(p2, "%lu", score2);
    graphics_draw_text(disp, SCREEN_WIDTH/2 + 20*.75, 10, p2);
}
...
int main(void)
{
    display_init(RESOLUTION, DEPTH_32_BPP, 2, GAMMA_CORRECT_DITHER, FILTERS_RESAMPLE_ANTIALIAS);
    ...
    while (1) {
        ...
        draw_scores(disp, paddles[0].score, paddles[1].score);
        draw_middle_line(disp);
        draw_rect(ball.pos.x, ball.pos.y, BALL_SIZE, BALL_SIZE, COLOR_WHITE);
        draw_rect(paddles[0].pos.x, paddles[0].pos.y, PADDLE_WIDTH, PADDLE_HEIGHT, COLOR_WHITE);
        draw_rect(paddles[1].pos.x, paddles[1].pos.y, PADDLE_WIDTH, PADDLE_HEIGHT, COLOR_WHITE);
        ...
        switch (g_game_state) {
            case Game_State_Playing:
            {
                ...
                // Ball collision with the ceiling and the floor.
